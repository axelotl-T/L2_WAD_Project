...
function switchView(viewName) {
  ...
  if (viewName === "dashboard") {
    ...
    $("#view-cart").show();
    renderCart();
    loadUserProfile(); // <--- CALLING PROFILE LOAD
  }
}
...
// ... (Rest of logic: Auth, Cart, Admin, Shop - Same as before) ...
function login() {
...
function renderCart() {
  ...
  if (cart.length === 0) {
    ...
    cart.forEach((item, index) => {
      ...
      total += itemTotal;
      container.append(
        `<div class="cart-item"><div style="flex-grow:1"><b>${item.name}</b><br>Unit Price: $${item.price}</div><div style="display:flex; align-items:center;"><button class="qty-btn btn-secondary" onclick="changeQty(${index}, -1)">-</button><span class="qty-val">${item.qty}</span><button class="qty-btn btn-success" onclick="changeQty(${index}, 1)">+</button></div><div style="width:100px; text-align:right; font-weight:bold;">$${itemTotal.toFixed(2)}</div><button class="btn btn-danger" style="margin-left:10px" onclick="changeQty(${index}, -9999)">X</button></div>`,
      );
    });
  }
...
function loadAdminProducts() {
  $.get("/api/products", function (products) {
    ...
    products.forEach((p) => {
      container.append(
        `<div class="card"><h4>${p.name} ($${p.price})</h4><p>${p.description}</p><p>Cat: <b>${p.category}</b> | Stock: ${p.stock}</p><button class="btn btn-warning" onclick="editProduct('${p._id}')">Edit Product</button><button class="btn btn-danger" onclick="deleteProduct('${p._id}')">Delete Product</button><button class="btn btn-info" onclick="$('#admin-rev-${p._id}').slideToggle()">Manage Reviews</button><div id="admin-rev-${p._id}" class="reviews-container"><h5>Reviews</h5><div id="admin-rev-list-${p._id}">Loading...</div><hr><small>Add Admin Comment:</small><div style="margin-top:5px;"><input type="text" id="admin-rev-txt-${p._id}" placeholder="Comment" style="width:60%"><input type="number" id="admin-rev-rate-${p._id}" placeholder="1-5" style="width:15%"><button class="btn btn-sm btn-primary" onclick="postReview('${p._id}', false)">Post</button></div></div></div>`,
      );
...
function loadAdminReviews(pid) {
  $.get(`/api/products/${pid}/reviews`, function (reviews) {
    ...
    reviews.forEach((r) => {
      ...
      div.append(
        `<div id="rev-item-${r._id}" class="admin-review-item"><div style="display:flex; justify-content:space-between;"><span><b>${userDisplay}</b>: ${r.comment} (${r.rating}/5)</span><div><button class="btn btn-sm btn-secondary" onclick="editReviewUI('${r._id}', '${r.comment}', ${r.rating}, '${pid}')">Edit</button><button class="btn btn-sm btn-danger" onclick="deleteReview('${r._id}', '${pid}')">Del</button></div></div></div>`,
      );
    });
  });
}
...
function editReviewUI(rid, oldComment, oldRating, pid) {
  let html = `<input type="text" id="edit-txt-${rid}" value="${oldComment}" style="width:50%"><input type="number" id="edit-rate-${rid}" value="${oldRating}" style="width:15%"><button class="btn btn-sm btn-success" onclick="saveReview('${rid}', '${pid}')">Save</button><button class="btn btn-sm btn-secondary" onclick="loadAdminReviews('${pid}')">Cancel</button>`;
...
function loadShopProducts() {
  ...
  $.get(url, function (products) {
    ...
    products.forEach((p) => {
      container.append(
        `<div class="shop-card" style="border:1px solid #eee; padding:15px; margin:5px; background:white;"><h4>${p.name}</h4><p>${p.description}</p><div class="price-tag">$${p.price}</div><button class="btn btn-primary" onclick="addToCart('${p._id}', '${p.name}', ${p.price})">Add to Cart</button><button class="btn btn-info" onclick="$('#shop-rev-${p._id}').slideToggle()">Show Reviews</button><div id="shop-rev-${p._id}" class="reviews-container"><small><b>Write a Review:</b></small><br><input type="text" id="shop-rev-txt-${p._id}" placeholder="Comment" style="width:60%"><input type="number" id="shop-rev-rate-${p._id}" placeholder="1-5" style="width:20%"><button class="btn btn-sm btn-success" onclick="postReview('${p._id}', true)">Post</button><hr><div id="shop-rev-list-${p._id}">Loading...</div></div></div>`,
      );
